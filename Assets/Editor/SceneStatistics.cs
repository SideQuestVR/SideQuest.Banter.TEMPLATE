    ...
    public static void DrawGUI(SceneView view)
    {
        if (ShowStatistics)
        {
            if (!validSelection)
                return;
        Handles.BeginGUI();
        // Calculate the position based on the Scene View size to align it bottom left
        Vector2 overlaySize = new Vector2(180, 90);
        Rect rect = new Rect(5, Screen.height - overlaySize.y - 50, overlaySize.x, overlaySize.y);
